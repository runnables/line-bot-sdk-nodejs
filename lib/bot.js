...
module.exports.init = function(config) {
  ...
  if (config.channelAccessToken) {
    ...
    defaultHeaders['X-LINE-ChannelToken'] = config.channelAccessToken;
  } else {
    // For Trial
  ...
  api.sendMultipleMessages = function(mid, multipleMessages, messageNotified) {
    return this.sendMessage(mid, MessageBuilder.buildMultipleMessages(multipleMessages, messageNotified || 0), null, EventType.SENDING_MULTIPLE_MESSAGES);
  };
  ...
  api.validateSignature = function(json, signature) {
    SignatureValidator.validateSignature(json, this.channelSecret, signature);
  };
  ...
  api.leaveGroup = function(groupId) {
    if (!config.channelAccessToken) throw new Error('Leave a group is only appear in Business Connect API');
  ...
  api.addOfficialAccount = function(accessToken) {
    if (!config.channelAccessToken) throw new Error('Add an official account to a user\'s friend list is only appear in Business Connect API');
